...
class BayesianTracker {
  ...
  runMonteCarloForecast(nRuns) {
    ...
    return {
        agiYears, asiYears,
        trajectory: { years: yrs, median: med, p10: p10a, p25: p25a, p75: p75a, p90: p90a, agiThreshold: 10, asiThreshold: 15 }
    };
  }
...
function plotScenarioFan(tracker) {
  ...
  traces.push(
    ...
    { x: yrRange, y: [15, 15], type: 'scatter', mode: 'lines', name: t.ch_legend_asi, line: { color: '#ef4444', dash: 'dot', width: 1 } }
  );

  Plotly.newPlot('c6', traces, {
    ...LAYOUT_BASE,
    xaxis: { ...LAYOUT_BASE.xaxis, title: { text: t.ch2_xlabel }, range: yrRange },
    yaxis: { ...LAYOUT_BASE.yaxis, type: 'log', range: [0.7, 1.4], title: { text: 'Capability (log)' } },
  }, PLOT_CFG);
}
...
const LANG = {
  ru: {
    ...
    arch_mc_desc:'3000 прогонов из апостериорного распределения. Каждый прогон — симуляция от 2023 до 2068 года с месячным шагом. Результат: распределение лет до AGI (cap >= 10) и ASI (cap >= 15).',
    defs_intro:'В модели v3 используются строгие операциональные определения на основе двухмерной шкалы (Reasoning, Agency). Шкала логарифмическая: GPT-4 (конец 2023) ~ 3.0 по Reasoning и ~0.2 по Agency, текущие модели середины 2026 ~ 6.8 по Reasoning и ~7.2 по Agency. AGI = 10.0, ASI = 15.0.',
    ...
    asi_def_score:'min(Reasoning, Agency) = 15.0',
  ...
  en: {
    ...
    defs_intro:'The v3 model uses strict operational definitions based on a two-dimensional scale (Reasoning, Agency). The scale is logarithmic: GPT-4 (late 2023) ~ 3.0 in Reasoning and ~0.2 in Agency, current mid-2026 models ~ 6.8 in Reasoning and ~7.2 in Agency. AGI = 10.0, ASI = 15.0.',
    agi_def_title:'AGI — Artificial General Intelligence',
    agi_def_score:'min(Reasoning, Agency) = 10.0',
